...
from flaskblog.models import User_table, Post
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User_table(username=form.username.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
...
def logout():
    logout_user()
    flash('You have been logged out. Kindly login again to continue.', 'success')
    return redirect(url_for('main.home'))
...
def user_posts(username):
    ...
    user = User_table.query.filter_by(username=username).first_or_404()
